...
//            std::cout << "valuetest: " << value << '\n';
    ...
    public:
        ...
        BoundaryCache(StepFunctional const& f,
                      typename AnsatzVars::VariableSet const& vars_, int flags=7):
            vars(vars_), dirichlet_penalty(f.gamma),  J(f.J),   up {0, 0, 1},  side {0,  1, 0}
        {
...
//                            alpha = 0.0;
        ...
        {
        	// c' = I''
            if(row == yIdx) return (alpha*arg.value) * p;
            if(row == uIdx) return  beta *( J.template d1<uIdx>(arg) -p*arg.value);
            if(row == pIdx) return alpha*y*arg.value - (beta* u*arg.value) ;
        ...
        Scalar d2_impl (VariationalArg<Scalar,dim,TestVars::template Components<row>::m> const &arg1, VariationalArg<Scalar,dim,AnsatzVars::template Components<col>::m> const &arg2) const
        {
            ...
            if (row==yIdx && col==pIdx) return alpha*(arg1.value * arg2.value);

            if(row==pIdx && col==yIdx)  return alpha*(arg1.value * arg2.value);
...
//         Dune::FieldVector<Scalar,AnsatzVars::template Components<yIdx>::m> u;
    ...
    explicit StepFunctional(Scalar regularization, Reference const& ref,  Integrand integrand) :
        gamma(1e9), alpha(0.), J(regularization,ref), integrand_(integrand)
    {
