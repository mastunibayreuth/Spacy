...
template<class Integrand, class VarSet, int stateId, int controlId, int adjointId>
class ParameterFunctional: public FunctionalBase<VariationalFunctional>
{
public:
	using Scalar = double;
	using OriginVars = VarSet;
	...
	using DomainCache = DomainCacheParameter<ParameterFunctional, Integrand, yIdx>;

	class BoundaryCache: public CacheBase<ParameterFunctional, BoundaryCache>
	{
	public:
		using FaceIterator = typename AnsatzVars::Grid::LeafIntersectionIterator;

		BoundaryCache(ParameterFunctional const& f_,
				typename AnsatzVars::VariableSet const& vars_, int flags = 7) :
	...
	private:
		...
		Scalar alpha;
		FaceIterator const* e;
		double localPenalty_, beta;
		const Vector up, side;
	};

	Integrand f_;

	explicit ParameterFunctional(Integrand f, double normalComplianceParameter) :
...
//       y_ = at_c<state_index>(y.data).value(at_c<y_Idx>(evaluators));
	...
	// use std::move ??
	explicit NonlinearElasticityFunctionalControl(Integrand f) :
			f_(f)
	{
	}
private:
