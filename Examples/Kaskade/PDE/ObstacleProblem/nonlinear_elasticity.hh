...
//                            alpha = 0.0;
        ...
        {
        	// c' = I''
            if(row == yIdx) return (alpha*arg.value) * p + localPenalty_* 3.0 * (obstacle_ - y[2])*(obstacle_ - y[2]) * p[2] * arg.value[2];
            if(row == uIdx) return  beta *( J.template d1<uIdx>(arg) -p*arg.value);
            if(row == pIdx) return alpha*y*arg.value - (beta* u*arg.value) - (localPenalty_ * (obstacle_ - y[2]) * (obstacle_ - y[2])*(obstacle_ - y[2]) * arg.value[2]);
        ...
        Scalar d2_impl (VariationalArg<Scalar,dim,TestVars::template Components<row>::m> const &arg1, VariationalArg<Scalar,dim,AnsatzVars::template Components<col>::m> const &arg2) const
        {
        	...
        	if(row == yIdx && col == yIdx)
        		...
        		// check again  dirichlet term missing // is correct
        	                if(role == RoleOfFunctional::TANGENTIAL )
                                return - 6.0 *(obstacle_ - y[2]) * (arg2.value[2])* localPenalty_ * p[2] * arg1.value[2];
        	                else
        	                {
        	                    if(role == RoleOfFunctional::NORMAL )
        	                    {
            ...
            if (row==yIdx && col==pIdx) return alpha*(arg1.value * arg2.value) + (localPenalty_ * 3.0 * (obstacle_ - y[2])*(obstacle_ - y[2]) * arg1.value[2] *arg2.value[2]);

            if(row==pIdx && col==yIdx)  return alpha*(arg1.value * arg2.value) + (localPenalty_ * 3.0 * (obstacle_ - y[2]) * (obstacle_ - y[2]) *arg1.value[2] *arg2.value[2]);
