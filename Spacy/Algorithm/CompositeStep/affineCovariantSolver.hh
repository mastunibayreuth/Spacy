...
namespace Spacy
{
  ...
  namespace CompositeStep
  {
    ...
    public:
      ...
      std::tuple<bool, Vector, Real, Real> solvePath(const Vector & x0, const Real & lambda, const Real & thetaGlobal );

      C2Functional & getNormalFunctional();
      C2Functional & getTangentialFunctional();

      void setNormalFunctional( C2Functional N);
      void setTangentialFunctional(C2Functional L);
    ...
    private:
      ...
      LipschitzConstant omegaL = LipschitzConstant( Real(1e-16) ) , omegaC = LipschitzConstant( Real(1e-6) );
